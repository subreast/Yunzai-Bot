...
export default class MysInfo {
   ...
   * @returns {Promise<*>}
   */
  async geetest (callbackUrl) {
    let uid = this.uid
    if (!uid && this.uid && this.uid.length > 0) {
      uid = this.uid[0]
    }
    let mysApi = new MysApi(uid, this.ckInfo.ck, { log: true },this.e.isSr)
    let res
    try {
      ...
      if (challenge) {
        res = await mysApi.getData(callbackUrl, {
