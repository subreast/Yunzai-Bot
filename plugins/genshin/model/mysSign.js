...
export default class MysSign extends base {
  ...
  async signTask(manual) {
    ...
    for (let i in uids) {
      ...
      let retry = 0;
      while (ret.retcode !== 0 && ret.retcode !== -100 && retry < 5) {
        ret = await this.doSign(ck, false);
        retry++;
      }
      if (ret.retcode === 0) {
        if (ret.is_sign) {
      ...
      if (this.signApi) {
        let rea = await fetch(`https://api.fuckmys.tk/token?token=${this.token}`);
        rea = await rea.json();
        if (rea.times > 2333) {
          await common.sleep(1000);
        } else {
  ...
  async signClose() {
    ...
    if (lodash.isEmpty(ck)) {
      await this.e.reply(`${model}签到失败，请先绑定cookie\n发送【#ck帮助】查看绑定教程`, false, { at: true })
      return false
    }
